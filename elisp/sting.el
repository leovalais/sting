...
(global-set-key (kbd "C-c s l l") 'sting-load-all-tests)
(global-set-key (kbd "C-c s l a") 'sting-load-all-tests)
...
(defface sting-panic-face '((t :foreground "#ff453a")) "")
    ...
    (etypecase report
      ...
      (sting-fail-report
       (ecase (sting-fail-report-kind report)
         ...
         (:panic
          (insert-newline)
          (sting-insert-property "Error"
                                 (propertize (sting-fail-report-error report)
                                             'face 'sting-panic-face)))
         (:assertion
          (insert-newline)
  ...
  (let ((report (sting-get-report test)))
    (insert (etypecase report
              ...
              (sting-fail-report (ecase (sting-fail-report-kind report)
                                   ...
                                   (:panic (propertize sting-panic-indicator 'face 'sting-panic-face)) ))))
    ...
    (if reports
        (let* ((passed (count-if #'sting-pass-report-p reports))
               (assertions (+ (count-if #'sting-failed-report-assertion-p reports)
                              (count-if #'sting-failed-report-panic-p reports)))
